...
### ⚡ Ключевые оптимизации v2.11.0

- `runner_len` снижен с 50 до 10 — меньше нагрузки на API FunPay
- Кэш узлов чатов (`__chat_nodes`) — повторные запросы заменены кэшем
- Фикс `saved_orders` — устранены ложные события о новых заказах при старте
