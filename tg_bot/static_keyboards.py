...
def SETTINGS_SECTIONS() -> K:
    return K()        .row(B(_("mm_language"), callback_data=f"{CBT.CATEGORY}:lang"),
             B(_("mm_global"), callback_data=f"{CBT.CATEGORY}:main"))        .row(B(_("mm_authorized_users"), callback_data=f"{CBT.AUTHORIZED_USERS}:0"),
             B(_("mm_proxy"), callback_data=f"{CBT.PROXY}:0"))        .row(B(_("mm_autodelivery"), callback_data=f"{CBT.CATEGORY}:ad"),
             B(_("mm_autoresponse"), callback_data=f"{CBT.CATEGORY}:ar"))        .row(B(_("mm_templates"), callback_data=f"{CBT.TMPLT_LIST}:0"),
             B(_("mm_plugins"), callback_data=f"{CBT.PLUGINS_LIST}:0"))        .row(B(_("mm_lots_editor"), callback_data=f"{CBT.FP_LOT_EDIT_LIST}:0"))        .add(B(_("gl_next"), callback_data=CBT.MAIN2))
...
def SETTINGS_SECTIONS_3() -> K:
    ...
    return K()        .row(B("📊 Графики статистики", callback_data="graphs_Settings"),
             B("🔄 Синхронизация чатов", callback_data="sync.settings"))        .row(B("💬 Ответы на отзывы", callback_data="ReviewChatReply_Settings"),
             B("📈 Лимиты рейтинга", callback_data="sras_info.settings"))        .add(B(_("gl_back"), callback_data=CBT.MAIN2))
...
def CONFIGS_UPLOADER() -> K:
    return K()        .row(B(_("cfg_download_main"), callback_data=f"{CBT.DOWNLOAD_CFG}:main"),
             ...
             B(_("cfg_upload_ad"), callback_data="upload_auto_delivery_config"))        .add(B(_("gl_back"), callback_data=CBT.MAIN2))
