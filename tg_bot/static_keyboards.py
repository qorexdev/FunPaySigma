...
def SETTINGS_SECTIONS() -> K:
    return K()\
        ...
        .row(B(_("mm_lots_editor"), callback_data=CBT.LE_SEARCH_MENU))\
...
def SETTINGS_SECTIONS_2() -> K:
    return K()\
        ...
        .row(B(_("mm_order_reminders"), callback_data=f"{CBT.CATEGORY}:or"),
             B(_("mm_review_reminders"), callback_data=f"{CBT.CATEGORY}:revr"))\
        .row(B(_("mm_review_reply"), callback_data=f"{CBT.CATEGORY}:rr"),
             B(_("mm_review_chat_reply"), callback_data="ReviewChatReply_Settings"))\
